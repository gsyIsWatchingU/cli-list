...
- 用自然语言描述需求，由 AI 生成增量指令，预览后再修改个人命令。
- 支持关键词搜索、标签筛选，并可按使用次数或最近使用排序。
- 支持一键置顶常用命令，置顶项始终排在列表最前。
...
- 启动时后台静默检查 GitHub Release；托盘与主窗口也可随时手动检查更新。
...
程序每次启动都会在后台静默检查更新；命令面板和托盘始终提供“检查更新”入口。发现新版本后，入口会显示目标版本；点击即可校验、切换并自动重启，失败时自动恢复旧版本。用户命令配置与使用统计不会被覆盖。
...
源码方式安装到 `%USERPROFILE%\.cli-list`。发现新版本后点击更新入口，程序会对 `main` 执行安全的快进更新、测试并同步安装版。仓库存在未提交修改时会停止更新，不会覆盖本地代码。

### 源码开发模式（自动同步本地改动）

桌面快捷方式、右键菜单都带 `--dev-sync` 参数，启动时会先检查源码工作区：

- 如果 `CLIList.cs`、`cli-list.ico` 等比安装版新，自动重新编译并同步到 `%USERPROFILE%\.cli-list`，然后用新版本启动。
- 源码没有变化时直接启动，不会重复编译（约 1 秒内完成判断）。
- 同步过程带文件锁，多个入口同时启动只会编译一次。
- 编译失败不会阻塞启动，会回落到现有安装版，保证面板仍然可用。

也就是说，改完源码后直接双击桌面快捷方式即可生效，无需手动执行 `build.ps1` 和 `sync-installed.ps1`。

同步逻辑实现在 `dev-sync.ps1`，源码路径优先读取安装目录的 `.source-repository` 标记（由 `install.ps1` 写入），标记缺失时回落到常见路径。
...
普通用户点击主界面的“增/删/改命令”即可完成配置：

1. 用不超过 300 字的自然语言描述要新增、修改或隐藏的命令，点击“复制给 AI”。
2. 把提示词发给任意大模型，再将模型返回的 JSON 粘贴回应用。
3. 查看增量修改预览，可直接优化命令名称，确认后应用。

应用只接受最多 10 条增量操作，不允许 AI 改写 `Id`、内置动作或整份配置。任何一条不合法都不会写入；修改仅保存到个人 `commands.local.json`。每次应用前会自动留存版本快照，最多保留最近 5 个，可在窗口左下角“版本历史”中查看、在多个版本之间来回切换。
...
项目已经可以构建、安装和日常使用，当前重点是安装稳定性与发布质量。功能状态和后续计划见：
